...
if __name__ == '__main__':
    ...
    parser.add_argument('--print_dialects', '-d', action='store', type=str, nargs='*', default=dialects, help='Generate lexicon files for one or more specified dialects.')
    parser.add_argument('--print_base', '-b', action='store_true', help='Generate a base lexicon file, containing the state of the lexicon prior to updates.')
    ...
    for d in args.print_dialects:
        with open(f'{d}.txt', 'w') as expfile:
            for elm in exp[d]:
                expfile.write(f'{elm[1]}\t{elm[2]}\t{elm[3]}\t{elm[-2]}\n')
        if args.print_base:
            base = get_base(connection)
            with open(f'../data/output/base.txt', 'w') as basefile:
                for el in base:
